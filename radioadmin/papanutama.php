<?php
	if($_POST){
	  $q_select = "SELECT * FROM USERSCODE WHERE USE_USERSCODE = '". addslashes($_POST['usr_identiti']) ."' AND USE_PASSWORDS = '". $_POST['usr_password'] ."'";
		$select = oci_parse($conn,$q_select);
		oci_execute($select);
	    $row = oci_fetch_assoc($select);
		if($row['USE_USESTATUS']) {
        $_SESSION['usr_identiti'] = $row['USE_USERSCODE'];
        $_SESSION['usr_namaindv']= $row['USE_SHORTNAME'];
        $_SESSION['logged']=true;
        echo '<META HTTP-EQUIV="Refresh" CONTENT="0.01; URL=./?home">';
		} else {
        echo '
            <script type="text/javascript">
            showNotification({
            message:"ID pengguna dan katalaluan tidak sepadan",
            type: "error",
            autoClose: true,
            duration: 3
            });
